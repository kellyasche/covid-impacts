...
# Number of establishments{.tabset}

Summary here

<br>

```{r prep qcew estab data, include=FALSE, cache=TRUE}
qcew.estab.county <- master.qcew.county %>%
  filter(key == "avg_estab") %>%
  droplevels() %>%
  group_by(countyfp, key) %>%
  mutate(index = (value - value[year.quarter == min(.$year.quarter)]) / value[year.quarter == min(.$year.quarter)]) %>%
  ungroup() %>%
...
qcew.estab.ruca <- master.qcew.ruca %>%
  filter(key == "avg_estab") %>%
  droplevels() %>%
  group_by(Dem_Desc, key) %>%
  mutate(index = (value - value[year.quarter == min(.$year.quarter)]) / value[year.quarter == min(.$year.quarter)]) %>%
...
qcew.estab.edr <- master.qcew.edr %>%
  filter(key == "avg_estab") %>%
  droplevels() %>%
  group_by(edr, planning.region, key) %>%
  mutate(index = (value - value[year.quarter == min(.$year.quarter)]) / value[year.quarter == min(.$year.quarter)]) %>%
...
```{r chart establishment index ruca}
qcew.estab.ruca.plot <- ggplot(qcew.estab.ruca, aes(year.quarter, index, color = Dem_Desc)) +
  geom_hline(yintercept = 0, color = "black") +
  geom_smooth(se = FALSE, size = 3) +
  geom_point_interactive(size = 5, aes(data_id = data_id, tooltip = paste(Dem_Desc, "\nYear and quarter: ", year, "\nTotal acres: ", comma(Total.acres, accuracy = 1.0), "\nTotal estimated value of ag land: ", dollar(Total.estimated.value), "\nEstimated value per acre: ", dollar(Estimated.value.per.acre), "\nTotal farm net income: ", dollar(farm.net.income), "\nFarm net income per acre: ", dollar(farm.net.income.acre), "\n", index.type, ": ", percent(index, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = "Change in values since 2000")+
  geom_label(data = filter(farm.net.inc.land.values.index.ruca, Year == 2019), aes(label = percent(index)), show.legend = FALSE, size = 5) +
  scale_y_continuous(labels=scales::percent)+
  scale_x_continuous(breaks = seq(1900, 2050, 2),
                     guide = guide_axis(n.dodge = 2)) +
  theme_line+
  ...
  girafe(ggobj = ag.pct.prop.rev.ruca.plot, width_svg = 10, height_svg = 7) %>%
    girafe_options(opts_selection(type = "none"),
                   opts_toolbar(saveaspng = FALSE),
                   opts_sizing(rescale = FALSE))
