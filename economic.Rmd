...
names(lf.annual.change.pr)

```

```{r chart laborforce trends}
lf.annual.change.pr.plot <- ggplot(lf.annual.change.pr, aes(year, annual.change, color = planning.region, group = planning.region)) +
  geom_col_interactive(size = 5, aes(data_id = data_id, tooltip = paste(planning.region, "\nYear: ", year, "\nAverage labor force: ", comma(avg.lf, accuracy = 1), "\nChange in average labor force since 2010: ", percent(avg.lf.index, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = "Change in average labor force since 2010")+
  geom_label_repel(data = filter(lf.avg.pr, year == max(year)), aes(label = percent(avg.lf.index, accuracy = .1)), show.legend = FALSE, size = 5) +
  scale_y_continuous(labels=scales::percent) +
  ...
  girafe(ggobj = lf.avg.pr.plot, width_svg = 10, height_svg = 7) %>%
    girafe_options(opts_selection(type = "none"),
                   opts_toolbar(saveaspng = FALSE),
                   opts_sizing(rescale = FALSE))
