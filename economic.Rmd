...
# Local Options Sales Tax{.tabset}
...
lost.cities <- master.lost.cities %>%
  ...
  group_by(city, planning.region, year) %>%
  mutate(num.months = n()) %>%
  ungroup() %>%
  filter(num.months == 12) %>%
  group_by(city, planning.region, year) %>%
  summarise(local.sales.tax.revenue = sum(local.sales.tax.revenue)) %>%
  ungroup() %>%
  group_by(city, planning.region) %>%
  mutate(count = n()) %>%
  ungroup() %>%
  filter(count > 9) %>%
  group_by(city, planning.region) %>%
  mutate(min.year = min(year),
         local.sales.tax.revenue.index = (local.sales.tax.revenue - local.sales.tax.revenue[year == min.year]) / local.sales.tax.revenue[year == min.year]) %>%
  ungroup() %>%
  mutate(data_id = seq(n())) %>%
  filter(min.year %in% c(2004, 2007))

lost.counties <- master.lost.counties %>%
  filter(`Year Imposed` == min(.$`Year Imposed`),
         year >2014) %>%
  group_by(County, countyfp, year) %>%
  summarise(local.sales.tax.revenue = sum(local.sales.tax.revenue, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(County, countyfp) %>%
  mutate(local.sales.tax.revenue.index = (local.sales.tax.revenue - local.sales.tax.revenue[year == min(.$year)]) / local.sales.tax.revenue[year == min(.$year)]) %>%
  ungroup() %>%
  mutate(data_id = seq(n()))
...
```{r chart LOST cities}
lost.cities.plot <- ggplot(filter(lost.cities, min.year %in% c(2004,2007), city != "Worthington"), aes(year, local.sales.tax.revenue.index, color = city)) +
  facet_wrap(~planning.region, ncol = 2) +
  geom_hline(yintercept = 0, color = "black") +
  geom_smooth(se = FALSE, size = 3) +
  geom_point_interactive(size = 5, aes(data_id = data_id, tooltip = paste(city, "\nYear: ", year, "\nYear tax was implemented: ", min.year, "\nLocal sales tax revenue: ", dollar(local.sales.tax.revenue), "\nChange in revenue since implementation: ", percent(local.sales.tax.revenue.index, accuracy = .1), sep = ""))) +
  geom_label(data = filter(lost.cities, year == 2019, city != "Worthington"), aes(label = percent(local.sales.tax.revenue.index, accuracy = .1)), show.legend = FALSE, size = 5) +
  labs(x="", y = "", color="", title = "Change in local option sales tax revenue since implementation") +
  scale_y_continuous(labels=scales::percent)+
  scale_x_continuous(breaks = seq(1900, 2050, 2),
                     guide = guide_axis(n.dodge = 2)) +
  theme_line+
