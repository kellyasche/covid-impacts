...
Variability in bed availability.
...
bed.var.ruca <- hosp.cap.ruca %>%
  group_by(Dem_Desc) %>%
  mutate(inpatient_beds_7_day_avg_index = (inpatient_beds_7_day_avg - inpatient_beds_7_day_avg[collection_week == min(collection_week)]) / inpatient_beds_7_day_avg[collection_week == min(collection_week)],
         total_staffed_adult_icu_beds_7_day_avg_index = (total_staffed_adult_icu_beds_7_day_avg - total_staffed_adult_icu_beds_7_day_avg[collection_week == min(collection_week)]) / total_staffed_adult_icu_beds_7_day_avg[collection_week == min(collection_week)]) %>%
  ungroup() %>%
  mutate(data_id = seq(n())) %>%
  gather(key = "index.type", value = "index", 12:13)
...
bed.var.ruca.plot <- ggplot(bed.var.ruca, aes(collection_week, index, color = Dem_Desc)) +
  ...
  geom_point_interactive(size = 5, aes(data_id = data_id, tooltip = paste(Dem_Desc, "\nDate: ", collection_week, "\nBed type: ", index.type, "\nChange in bed type since ", min(bed.var.ruca$collection_week), ": ", percent(index, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = paste("Change in values since ", min(bed.var.ruca$collection_week), sep = "")) +
  geom_label(data = filter(bed.var.ruca, collection_week == max(collection_week)), aes(label = percent(index, accuracy = .1)), show.legend = FALSE, size = 5) +
  scale_y_continuous(labels=scales::percent) +
  theme_line +
  scale_color_manual(values = color.ruca,
                     guide = guide_legend(ncol = 3)) +
